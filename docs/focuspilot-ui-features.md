...
erDiagram
    ...
    TASK {
        string status "backlog/todo/in_progress/in_review/done/blocked"
...
- **对话拆分/细化**："帮我拆分这个 Epic" → Engine 理解意图 → 调用合适能力 → 返回建议 → 用户确认/调整
- **自动执行**："把这个 Task 交给代码工程师做" → Engine 调度 Crew 成员执行 → 实时反馈
...
| 无模式 Agent 编排 | 项目树 + 对话面板 | 配执行/评估 Agent 自动调度，任意粒度均适用 |
