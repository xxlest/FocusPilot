...
| `ready` | 拖入「待办」即被中央扫描器自动调度执行（取代原 `auto_execute` 字段；详见 [Studio 执行模型](fp-ui/04-studio.md) §3） |
| `executing` | Agent 工作中，实时反馈进度 |
| 容器节点执行 | 递归执行其子节点中所有 Task（**V2**；V1 只执行当前节点自身，见 04-studio §3.1/§15） |

> **状态词汇对齐注**：PRD §3.1（`inbox/planning/ready/executing`）与 04-studio §2.4（`backlog/todo/in_progress/in_review`）是既存的两套状态词汇；本期只对齐 `auto_execute` 字段的去除（被"拖入待办即调度"取代），**完全统一两套词汇另起独立任务处理**。
...
- **Focus Bar（实时状态）**：菜单「实时状态」打开整页浮出的状态条，条内可点 × 关闭。状态项按 `待规划 · 运行中 · 待验收 · 已完成 ┃ 今日聚焦` 分组（执行流水在前、聚焦在后，中间分隔符），各状态用不同颜色标记、数字放大；每组 hover 下拉列任务。
...
- **任务字段内联编辑**：详情卡片的属性 chip 条中，`状态 / 优先级 / Agent / 调度 / 评估` 可点击就地弹下拉修改，状态遵循 Studio 7 态流转；系统自动流转（启动→执行中、完成→待验收）与手动修改走同一套机制。
