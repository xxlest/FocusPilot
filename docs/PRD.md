...
**差异化定位**：FocusPilot 是本地优先的智能 AIOS，唯一同时提供"四模式项目管理 + AI 规划与自动执行 + 窗口管理 + 番茄钟 + Today Dashboard"的桌面平台。
...
任意节点（Project / Epic / US / Task / 自定义层级）都支持「AI 辅助规划」+「自动执行」两类能力，**无需选择执行模式**：

- **AI 辅助规划**：通过对话面板辅助拆分子任务、细化描述、补全验收标准，人确认/调整。
- **自动执行**：任务配了执行 Agent，拖入「待办」即被中央扫描器自动调度；配了评估 Agent 则带「进行中 ⇄ 审核中」评估接力（详见 [04-studio §3 无模式化执行模型](fp-ui/04-studio.md)）。
- **递归**：容器节点执行 = 其子节点依次执行（**V2**）。
...
规划与执行都通过统一的**对话面板**进行，用户不需要感知 Skill、Agent 等技术概念：
- 规划：用户输入"帮我拆分这个 Epic" → Engine 理解意图 → 自动调用合适的能力 → 返回拆分建议 → 用户确认/调整
- 执行：用户输入"把这个 Task 交给代码工程师做" → Engine 理解意图 → 调度 Crew 成员执行 → 实时反馈
...
| **AI 规划 + 自动执行** | 对话拆分细化 + 配 Agent 自动调度（无模式） | 容器级递归自动执行 |
...
| AI 规划 + 自动执行 | 对话拆分细化（人+AI 协作）→ 配 Agent 自动执行 |
