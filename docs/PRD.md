...
Studio 任务视图把计划范围和显示模式拆成两条轴：侧边栏 Scope 决定 `全局规划 / 本月计划 / 本周计划 / 今日聚焦 / 执行中 / 等我决策 / 来源` 等范围；主区域默认显示 `▱ 甘特规划`，并通过低调下拉控件在 `▦ 看板 / ☰ 列表 / 田 泳道` 间切换。`时间 / 目标 / 来源` 是当前 Scope 内的二级筛选，分别按 `schedule`、`goal`、Workspace 类型过滤，并同步作用于看板、列表和泳道。切换显示模式不改变当前 Scope。看板主甬道固定显示为：待规划 / 待办 / 进行中 / 审核中 / 已完成 / 已阻塞；Task 卡片底部最后一行显示所属 Workspace 名称，便于跨项目扫描。看板可通过 `▤ Workspace` 开关切换为 Workspace 行分组：左侧增加 Workspace 名字列，右侧仍按 6 个状态列展示每个 Workspace 下的任务；该开关只在看板模式显示，离开看板自动关闭。泳道为 Workspace × 状态二维视图，允许拖拽卡片跨 Workspace 和状态单元格，drop 后同步改变任务归属与状态。
