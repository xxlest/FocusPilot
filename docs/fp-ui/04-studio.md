...
- **对话**：上述自动执行多轮对话流 + 追加指令输入框。
- **变更**：Agent 这次 Run 产出的**任务级变更复审面**。**两个 Tab 对所有任务常驻**，UI 一致：有产物时显示文件树 + diff/预览、Tab 上带文件数角标；无产物（纯手动卡片或尚未执行）显示空状态「该任务暂无 Agent 产出的变更」。看板卡片在有产物时显示 `📝 N 变更` 角标，点角标直接进入此视图。
- **裁决条按状态显隐**：仅「审核中」显示「通过并交付 / 打回继续 / 丢弃变更」按钮；其余状态显示只读说明（已完成→`✓ 已交付`、进行中→`⚡ 执行中`、已阻塞→`⛔ 已阻塞`）。
...
│ ┌ 变更文件树 ───────┬ Diff / 预览 ───────────────────────────────┐ │
│ │ M Models.swift +42-8                                            │ │
│ │ A KanbanData…  +186  ◀  → 右侧按文件类型自适应：                  │ │
│ │ A 执行报告.md  预览       · 修改文件 → unified diff（增删高亮）   │ │
│ └───────────────────┴      · 新建产物（md/图片）→ 直接预览        │ │
│ [✅ 通过并交付]  [↩ 打回继续]  [🗑 丢弃变更]                          │
└──────────────────────────────────────────────────────────────────┘
```

- **来源**：变更基于 ExecutionRun 的执行环境（git_project = worktree 分支 vs `base_commit`；local_directory = 工作区未提交变更；temporary = 全新增产物）。
- **同时支持 diff 与文件查看**：代码改动看 diff，报告/图片等新建产物直接预览，覆盖代码与非代码任务。
- **裁决闭环**（与「审核中」联动，任务落 in_review 时详情默认打开「变更」）：
  - **✅ 通过并交付** → 任务置 `done`（git 合并分支 / local 保留 / temp 导出），清除变更集
  - **↩ 打回继续** → 任务回 `in_progress`，回到对话视图触发下一轮接力
  - **🗑 丢弃变更** → 二次确认后放弃 worktree / 还原工作区，清除变更集
