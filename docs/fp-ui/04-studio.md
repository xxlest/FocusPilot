...
- **顶部筛选**：`目标 / 来源` 是当前计划范围内的二级筛选；目标按 WorkItem `goal` 过滤，来源按 Workspace 类型（本地项目 / Git 远程 / 临时创建）过滤，并同步作用于时间轴、看板、列表和泳道。时间范围不在顶部重复出现，统一由左侧 `全局规划 / 本月计划 / 本周计划 / 今日聚焦` 等 Scope 控制
