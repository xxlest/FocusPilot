...
**属性内联编辑 + 删除**：右侧属性区中可枚举字段（状态 / 优先级 / 安排 / 归属目标 / 执行 Agent / 标签）支持**点击就地弹下拉修改**（状态遵循 6 态流转，标签为多值 chip，可加可删）；改任意字段即时同步看板/泳道/列表全视图。详情头部提供 **🗑 删除** 按钮（二次确认后从所有视图移除）。Workspace / 项目 / 评估 / 截止暂为只读。
