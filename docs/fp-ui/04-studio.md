...
> **历史**：原 Focus 页面（[03-focus.md](03-focus.md)）已合并至本页；原 Studio 旧版规格已废弃（如需查阅，见 git 历史 `git show 59c3339:docs/fp-ui/04-studio-backup-v1.md`）
...
WorkItem:
  ...
  tags: ["backend", "架构"]      # 自由标签，详情面板可加/删（多值 chip）
  sort_order: 1024              # 同状态/同组内手动排序权重；拖拽排序时更新
...
**创建按钮落点**：仅保留单一「创建」按钮，落点由「初始状态」下拉决定——选「待规划」=创建后不调度；选「待办」=创建后进自动调度（无需独立的「创建并启动 / 创建并放入待办」按钮）。
...
- **重试触发与恢复**：`run_state ∈ {failed, timeout}` 的任务，经详情对话流的「🔄 重新生成」（最后一条 agent 回复操作栏，= `retryTask`；卡片不放按钮）触发。触发后清当前（已 aborted）Run 标记、`run_state → queued`、`status` **不变**（仍 in_progress），下轮扫描器在**保留的 worktree / 路径锁 + 同一 session** 上 resume 上下文、起一个**新 executor Run**（对齐 §3.3 派发范式，**非走 blocked**）。失败/超时**不释放** worktree / 路径锁（任务未离开执行流）。与 blocked 区分：blocked = 外部依赖（人工标/解除），failed/timeout = 引擎内失败（一键重试）。
...
│     实现看板 6 态拖拽 + 状态持久化…                         │
│  ──────────────────────────────────────────────────────── │
│  🤖 代码工程师 · 自动执行            00:03:42      📋 │  ← 入队/被拉起后第一个自动启动
│     修改 Models.swift…                                      │
...
**属性内联编辑 + 删除**：右侧属性区中可枚举字段（状态 / 优先级 / 安排 / 归属目标 / 执行 Agent / 标签）支持**点击就地弹下拉修改**（状态遵循 6 态流转，标签为多值 chip，可加可删）；改任意字段即时同步看板/泳道/列表全视图。详情头部提供 **🗑 删除** 按钮（二次确认后从所有视图移除）。Workspace / 项目 / 评估 / 截止暂为只读。详情对话流内**每条 agent 回复**末尾带操作栏：**📋 复制**（每条都有）+ **🔄 重新生成**（仅最后一条）。任务态「重新生成」= `retryTask`：`run_state→queued`，刷新详情对话流与各视图卡片 tint（`refreshStudioTaskViews`）；卡片本身**不放**按钮，仅整卡变色（详见 DesignGuide §5.10）。

#### 任务详情：对话 / 变更 / 文件 三视图

详情主区顶部提供 **`对话 | 变更 | 文件`** Tab，**三个 Tab 对所有任务常驻**，UI 一致：

- **对话**：自动执行多轮对话流 + 追加指令输入框。
- **变更**：Agent 这次 Run 产出的**任务级变更查看面**（只读，不含裁决按钮）。有产物时显示变更文件树 + diff/预览、Tab 上带文件数角标；无产物（纯手动卡片或尚未执行）显示空状态「该任务暂无 Agent 产出的变更」。看板卡片在有产物时显示 `📝 N 变更` 角标，点角标直达此视图。
- **文件**：浏览**所属 Workspace 的文件**（与本次变更无关，是整个工作区的文件树）。左侧目录/文件树，右侧选中文件预览（代码/Markdown 文本、图片占位）。

变更视图布局：

```
┌ 变更 · task/FP-002 @ 3764760 · 3 文件 +228 −12 · build✓ test✓ lint✓ ┐
│ ┌ 变更文件树 ───────┬ Diff / 预览 ───────────────────────────────┐ │
│ │ M Models.swift +42-8        → 右侧按文件类型自适应：             │ │
│ │ A KanbanData…  +186  ◀       · 修改文件 → unified diff（增删高亮）│ │
│ │ A 执行报告.md  预览          · 新建产物（md/图片）→ 直接预览      │ │
│ └───────────────────┴─────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────┘
```
...
```
┌ Workspace：📁 FocusPilot  ~/Workspace/FocusPilot ─────────────────┐
│ ┌ 文件树 ──────────┬ 预览 ──────────────────────────────────────┐ │
│ │ 📁 FocusPilot     │ // 选中文件内容（代码/Markdown 文本预览）   │ │
│ │  📃 Models.swift ◀ │                                            │ │
│ │ 📁 docs           │                                            │ │
│ │  📄 PRD.md        │                                            │ │
│ └──────────────────┴────────────────────────────────────────────┘ │
...
- **变更来源**：基于 ExecutionRun 的执行环境（git_project = worktree 分支 vs `base_commit`；local_directory = 工作区未提交变更；temporary = 全新增产物）。
- **同时支持 diff 与文件查看**：变更 Tab 看本次 diff/产物，文件 Tab 浏览所属 Workspace 全量文件，覆盖代码与非代码任务。
- 变更复审为**只读查看**，不在此处做通过/打回/丢弃裁决（任务状态流转仍由看板拖拽与对话回复驱动）。
...
> 无模式设计，与 §3.1「创建任务的执行配置」一致：不选执行模式，行为由「执行 Agent + 自动评估」隐式推导。
...
│                                                     │
│  ── 执行配置 ─────────────────────────────────────  │
│  执行 Agent: [代码工程师 ▾]   ← 留空=手动卡片         │
│  归属目标:   [4月/FP 0.0.1 ▾]  （可选）              │
│  ☑ 开启自动评估                                     │
│     评估 Agent: [质量审查员 ▾]   评估轮数: [− 3 +]   │
│                                                     │
│                       [取消]  [创建]                 │
└─────────────────────────────────────────────────────┘
```

落点由「初始状态」下拉决定（待规划=不调度 / 待办=进自动调度），不再有独立的「创建并启动」按钮（详见 §3.1）。
