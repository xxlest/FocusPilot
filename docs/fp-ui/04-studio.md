...
- **看板**：仅展示 executable/hybrid 角色 WorkItem，按 6 个主甬道分列（待规划 / 待办 / 进行中 / 审核中 / 已完成 / 已阻塞）；每张卡片底部最后一行显示所属 Workspace 名称；每个泳道标题右侧提供 `+` 新建任务；`▤ Workspace` 开关默认关闭，打开后看板左侧增加 Workspace 名字列，并按 Workspace 一行一行展示各状态列下的任务
