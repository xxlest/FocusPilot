...
- **计划范围**：由侧边栏 `今日聚焦 / 本周计划 / 本月计划 / 全局规划 / 执行中 / 等我决策 / 来源` 决定（时间范围按执行优先自上而下排列，今日聚焦置顶），并保持 `今日聚焦 ⊂ 本周计划 ⊂ 本月计划 ⊂ 全局规划` 的包含语义
- **视图模式**：用单一下拉控件切换，默认 `▦ 看板`；菜单上方分组为 `执行视图`（`▦ 看板 / 田 泳道 / ☰ 列表`），下方分组为 `规划视图`（`▱ 时间轴`）；切换视图模式不改变当前计划范围
- **工具栏稳定性**：全局工具栏固定只含 `视图下拉 / 目标 / 筛选 / 新建 Task`，切换任意视图模式都不增减控件；分组控件不再挂在工具栏，改为内嵌到各视图自身头部（详见时间轴、泳道），消除"切换视图时按钮布局突变"的突兀感
- **时间轴**：四级同构甘特（全局 月级 / 本月 周级 / 本周 天级 / 今日 小时级），用于目标树、长期排期和下钻；时间轴内容上方提供**内嵌头部分组控件**（`分组方式` 文字标签 + `▱ 按目标 / ▤ 按项目` 扁平下拉），按项目时以 Workspace/项目为行轴展示同一范围内任务
...
- **看板**：仅展示 executable/hybrid 角色 WorkItem，按 6 个主甬道分列（待规划 / 待办 / 进行中 / 审核中 / 已完成 / 已阻塞）；每张卡片底部最后一行显示所属 Workspace 名称；每个泳道标题右侧提供 `+` 新建任务；卡片可拖到其他状态列，drop 后只改变任务状态；看板视图无分组控件
- **列表**：全量表格化展示，行首提供复选框和全选，选中后显示已选数量，并支持批量修改状态、优先级、负责人和删除；列表视图无分组控件
- **泳道**：独立二维视图，网格上方提供**内嵌头部分组控件**（`分组方式` 文字标签 + `▤ Workspace / ◉ 执行 Agent / ↳ 父级任务 / ◎ 负责人` 扁平下拉，不再挂在全局工具栏）；每个分组行可折叠/展开；Workspace 分组时行是 Workspace，列是任务状态，drop 后同步改变任务状态与 Workspace 归属；Agent、父级任务、负责人分组时分别同步对应字段与状态
