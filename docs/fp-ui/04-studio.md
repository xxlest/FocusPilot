...
侧边栏顶部用 Tab 切换两种视角：`[📋 任务视图 | 📂 项目视图]`。同一时刻只显示一个 Tab 的内容，切换时侧边栏内容和主区域同步替换。`任务视图` 承载跨项目 WorkItem 的范围筛选与展示方式；`看板` 只作为任务展示方式出现，不再作为 Studio 顶层视角命名。
...
显示跨项目的任务视图。左侧侧边栏决定计划范围，主区域顶部将“甘特规划”和“展示方式”拆成两条轴：甘特用于长期目标与时间排布；看板 / 列表 / 泳道只是同一范围内 WorkItem 的呈现模式。
...
- **展示方式**：下拉切换 `看板 / 列表 / 泳道`；切换展示方式不改变当前计划范围
- **看板**：仅展示 executable/hybrid 角色 WorkItem，按 6 个主甬道分列（待规划 / 待办 / 进行中 / 审核中 / 已完成 / 已阻塞）；每张卡片底部最后一行显示所属 Workspace 名称；每个泳道标题右侧提供 `+` 新建任务和 `...` 操作菜单
- **列表**：全量表格化展示，支持多维排序和分组
- **泳道**：复用看板卡片信息，但按 Workspace 分组呈现，适合跨项目扫描同一范围内的任务归属
...
- 在项目任务列表中新建任务，同步出现在任务视图的对应展示方式中
