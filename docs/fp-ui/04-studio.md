...
侧边栏顶部用 Tab 切换两种视角：`[📋 看板视图 | 📂 项目视图]`。同一时刻只显示一个 Tab 的内容，切换时侧边栏内容和主区域同步替换。
...
### 7.2 场景 B：选中某个项目（未进入 Session）

主区域显示项目概览 + Session 列表，右上角提供可显隐的任务列表入口：

```
┌─ 主区域 ──────────────────────────────┬─ 右面板（可显隐）──────┐
│                                       │                       │
│  📂 FocusPilot         [📋 任务列表]  │  📋 项目任务列表       │
│  ──────────────────────────────────── │  ──────────────────── │
│                                       │  ● FP-002 看板拖拽 P0  │
│  对话列表                              │  ○ FP-003 列表视图 P1  │
│  ┌─────────────────────────────────┐ │  ✓ FP-001 数据模型     │
...
- 右面板通过顶栏 `[📋 任务列表]` 按钮可显隐
- 任务列表展示该 project_id 下的 WorkItem（从统一 workItems 数据源过滤）
- 点击任务 → 打开 Task 详情（场景 D）
- 在看板视图创建且绑定该 Project 的任务，同步出现在这里
