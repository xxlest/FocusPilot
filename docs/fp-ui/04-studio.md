...
显示跨项目的任务视图。左侧侧边栏决定计划范围，主区域顶部将“甘特规划”和“显示模式”拆成两条轴：甘特用于长期目标与时间排布；看板 / 列表 / 泳道只是同一范围内 WorkItem 的呈现模式。
...
- **甘特规划**：四级同构甘特（全局 月级 / 本月 周级 / 本周 天级 / 今日 小时级），默认进入，用于目标树、长期排期和下钻
- **显示模式**：用低调符号下拉控件切换 `▦ 看板 / ☰ 列表 / 田 泳道`；切换显示模式不改变当前计划范围
- **顶部筛选**：`时间 / 目标 / 来源` 是当前计划范围内的二级筛选；时间按 `schedule` 过滤，目标按 WorkItem `goal` 过滤，来源按 Workspace 类型（本地项目 / Git 远程 / 临时创建）过滤，并同步作用于看板、列表和泳道
- **看板**：仅展示 executable/hybrid 角色 WorkItem，按 6 个主甬道分列（待规划 / 待办 / 进行中 / 审核中 / 已完成 / 已阻塞）；每张卡片底部最后一行显示所属 Workspace 名称；每个泳道标题右侧提供 `+` 新建任务；`▤ Workspace` 开关默认关闭且仅在看板模式显示，打开后看板左侧增加 Workspace 名字列，并按 Workspace 一行一行展示各状态列下的任务；切到甘特 / 列表 / 泳道时自动关闭并隐藏
- **列表**：全量表格化展示，支持多维排序和分组
- **泳道**：独立 Workspace × 状态二维视图，行是 Workspace，列是任务状态；卡片可拖到任意 Workspace 和状态交叉单元，drop 后同步改变任务状态与 Workspace 归属
