...
显示跨项目的任务视图。左侧侧边栏决定计划范围，主区域顶部只保留一个视图模式下拉：执行视图（看板 / 泳道 / 列表）在上，规划视图（时间轴）在下；默认打开看板。
...
- **时间轴**：四级同构甘特（全局 月级 / 本月 周级 / 本周 天级 / 今日 小时级），用于目标树、长期排期和下钻
- **顶部筛选**：`时间 / 目标 / 来源` 是当前计划范围内的二级筛选；时间按 `schedule` 过滤，目标按 WorkItem `goal` 过滤，来源按 Workspace 类型（本地项目 / Git 远程 / 临时创建）过滤，并同步作用于时间轴、看板、列表和泳道
