...
- **泳道**：独立二维视图，顶部 `▤ Workspace / ◉ 执行 Agent` 分组控件只在泳道模式显示；Workspace 分组时行是 Workspace，列是任务状态，drop 后同步改变任务状态与 Workspace 归属；执行 Agent 分组时行是 Agent，drop 后同步改变任务状态与执行 Agent
