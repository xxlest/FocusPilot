...
- **看板**：仅展示 executable/hybrid 角色 WorkItem，按 6 个主甬道分列（待规划 / 待办 / 进行中 / 审核中 / 已完成 / 已阻塞）；每张卡片底部最后一行显示所属 Workspace 名称；**每个状态列标题栏右侧提供 `+` 作为唯一新建入口**（列底部不再放「＋ 新建任务」、顶部也不再有「新建 Task」按钮，避免功能重复）；从某列的 `+` 创建时，新建弹窗的「初始状态」自动预选该列状态（如在「进行中」列点 `+` 即默认 in_progress），不再一律落 backlog；卡片可拖到其他状态列（drop 改变任务状态），**在同一状态列内拖拽则按落点位置调整卡片顺序**（不改状态）；看板视图无分组控件；`run_state=failed/timeout` 的卡片底部常驻「↻ 重试」按钮（中性色，详见 §7.4 与 DesignGuide §5.10），列表/泳道卡片同款
...
**属性内联编辑 + 删除**：右侧属性区中可枚举字段（状态 / 优先级 / 安排 / 归属目标 / 执行 Agent / 标签）支持**点击就地弹下拉修改**（状态遵循 6 态流转，标签为多值 chip，可加可删）；改任意字段即时同步看板/泳道/列表全视图。详情头部提供 **🗑 删除** 按钮（二次确认后从所有视图移除）。Workspace / 项目 / 评估 / 截止暂为只读。当 `run_state=failed/timeout` 时，详情头部在「🗑 删除」旁显示「↻ 重试」（语义同 §3.2，中性色）；查看失败原因**复用对话视图**——失败 executor Run 行已带 `[日志]`，点击滚到该 Run 最后输出，不新造组件。
