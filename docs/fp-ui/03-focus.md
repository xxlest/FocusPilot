...
**三级同构甘特**：规划视图统一采用「左侧任务列表 + 右侧甘特时间轴」布局，左侧列表行与右侧甘特行**一一对齐**。三种模式仅时间粒度不同：
...
- 左侧列表行与右侧甘特行一一对齐；默认显示 7 天（周一~周日）
- 左列表分「已排期」和「未排期」两组，未排期任务右侧保留同高 drop row（不显示时间条），可作为拖拽排期目标
