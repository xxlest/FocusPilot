...
点击成员后，工作区切换到对应成员详情（默认显示概览 Tab）。

### 2.2 Runtime 分段

切到 Runtime 后展示本机、远程节点和云端执行环境，避免把"人"和"机器"平铺在同一列表里。
...
│                                 │
│ 检测范围                         │
│ 本机配置 自动检测 / 远程节点 需授权 / 执行器 5 在线 │
└─────────────────────────────────┘
```

点击机器后，工作区切换到对应的运行时监控页（单页布局，无 Tab 切换）。
...
| 7 天活动 | 柱状图展示最近 7 天运行分布，附 `趋势` pill | 点击柱状图某天筛选最近工作列表，再次点击取消筛选；选中柱高亮 + 日期标签加粗 + 最近工作标题旁显示筛选 pill（`周X ✕`） |
| 当前状态 | 正在执行的 `CrewRun`；无任务时显示空状态文案 | 有任务时点击进入运行详情 |
| 最近工作 | 默认展示最近 3 条；筛选某天时展示该天全部记录。显示 `状态图标 + Focus 编号 + Task 标题 + 时间 + 耗时 + Skill` | 点击行打开运行记录详情；点击 `全部记录` 打开完整列表；筛选 pill 可点击清除筛选 |
...
| Agent 并发上限（`max_concurrent_tasks`） | select | 1 / 2 / 4 / 8（默认 1）；该 Agent 同时能跑多少个任务（不同任务/Workspace 间并行）。属三层并发的**「Agent 层」**，区别于 Settings 的「Daemon 全局层」 |
...
Runtime 工作区为**执行端监控页**（单页滚动，无顶部 Tab），依次展示：Runtime Header → 执行器表格 → Daemon 日志。面包屑栏不显示分段控件。daemon 进程负责自动扫描本机 CLI、读取环境配置、维护健康度和日志，UI 只展示状态和提供操作控制，不暴露 Env / Args / MCP 等编辑配置（高级配置留给 Settings）。

面包屑格式：`AICrew / Runtime / MacBook-Pro-10.local`

#### 3.2.1 Runtime Header

Runtime 页面顶部信息栏（CSS class `crew-detail-header`），展示节点名、状态摘要、daemon 版本和操作按钮。

**空状态**：整页只显示一个空状态——节点名、未连接状态、引导文案、`配置连接` 和 `安装指南` 按钮。通过 `setRuntimeHostAvailability(isLocal)` 函数切换详情/空状态两个面板。

**控制状态渲染**：`renderRuntimeControlState()` 统一根据 `runtimeHostState[hostId].stopped` 渲染 pill、subline、表格健康度/工作负载、Restart 可见性和 Stop/Start 按钮文案。在 Stop/Start 操作后和节点切换时（`renderCrewView`）均调用。
...
Header subline 展示 daemon 版本（`daemon v0.3.11`）和最后心跳时间，不展示单个执行器的 CLI 版本（各执行器版本可能不同，CLI 版本可通过 tooltip、行展开或日志详情查看）。

操作按钮：

| 按钮 | 行为 |
|------|------|
| View logs | 平滑滚动到页面底部 Daemon 日志区 |
...
页面内联展示，位于执行器表格下方，用 `line-soft` 分隔线和"Daemon 日志"小标题标识。`刷新` 按钮手动刷新。Header 的 `View logs` 按钮滚动到此区域。
...
| 用户任务 | 操作路径 | 规则 |
|----------|----------|------|
| 查看成员详情 | 侧边栏点击成员 | 工作区切到该成员，默认概览 Tab |
| 新建智能体 | 侧边栏 `+ 新建智能体` 或模板行 | 弹窗含头像占位 + 名称 + 描述（255 字限制）+ 运行时 select + 模型 select（默认跟随 Runtime）+ 指令 textarea（默认收起，可展开）；个人版无可见性选项；创建后生成草稿成员 |
...
| 按天筛选工作记录 | 概览 Tab → 点击 7 天活动柱状图某天 | 最近工作列表筛选为该天记录，标题旁显示 `周X ✕` pill；再次点击同一天或点击 pill 清除筛选 |
| 查看动态 | 概览 Tab | 默认视图，展示 Hero 卡片、状态网格、活动图、当前工作和最近工作 |
| 查看运行记录 | 概览页 `查看记录` 或最近工作行 | 打开运行记录详情，包含工具调用、事件、配置快照和原始日志 |
| 过滤成功记录 | 点击成功率 | 打开记录列表并默认过滤 `status=success` |
| 查看 Runtime 详情 | 侧边栏 Runtime 分段 → 点击机器 | 单页监控视图：Header + 执行器表格 + Daemon 日志 |
| 查看 Daemon 日志 | Runtime Header → `View logs` 或直接滚动 | 滚动到底部日志区，`刷新` 按钮手动刷新 |
| 停止 Daemon | Runtime Header → `Stop` | 危险操作，确认后停止；隐藏 Restart，Stop 变为 `Start` |
| 重启 Daemon | Runtime Header → `Restart` | 状态 pill 显示"重启中…"，完成后恢复 |
| 添加执行节点 | 侧边栏 Runtime 分段 → `+ 添加执行节点` | 远程节点来自手动添加、历史连接或远程 daemon 心跳 |
...
crewState = {
  ...
  // runtimeHostState[hostId].stopped — 按节点存储 daemon 停止状态
}
```
...
- **Agent 并发上限（`max_concurrent_tasks`，默认 1）**：单个 Agent 同时能跑多少个任务；与 **Daemon 全局上限**（Settings，`daemon_max_concurrent_tasks`）、**Issue×Agent=1**（同 Agent 同任务最多 1，不暴露 UI）共同构成三层并发控制（详见 [04-studio §3.3](04-studio.md)）。Studio 中央扫描器抢槽时按此上限限流。
...
- 最近工作默认展示 3 条（概览页），完整记录列表展示全部可检索历史。
...
- 远程节点不由本机扫描磁盘；只展示远程 daemon 主动上报、用户手动添加、或历史连接留下的 Runtime 信息。
...
- 成员工作区三 Tab：概览 / Tasks / 配置
- 概览 Tab：Hero 卡片 + 状态网格 + 7 天活动柱状图 + 当前状态 + 最近工作
- Tasks Tab：按状态分组（进行中/待办/待规划/已完成），点击跳转 Focus
- 配置 Tab：子 Tab 切换（基础信息/指令/Skill/MCP/环境变量/自定义参数）
- Runtime 工作区：单页执行端监控（无顶部 Tab）
- Runtime Header：节点名 + 状态摘要 + daemon 版本 + 最后心跳 + View logs / Restart / Stop
- 执行器表格：4 列精简（执行器 / 健康度 / 智能体 / 工作负载）
- Daemon 日志区：内联日志列表 + 刷新按钮
- 远程节点未连接空状态
...
CrewMember:
  ...
  max_concurrent_tasks: 1      # Agent 并发上限（默认 1）；三层并发的「Agent 层」（§5.6）
