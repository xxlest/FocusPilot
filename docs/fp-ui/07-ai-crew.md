...
工作区采用 Multica 风格的轻量结构。顶部只服务当前成员；Runtime 作为基础设施对象放到侧边栏。

1. `动态`
2. `配置`

`Tasks`、`指令`、`Skills`、`环境变化` 不再作为一级 Tab 常驻展示：Task 和记录留在动态页；Instructions、Skill、Runtime 绑定进入配置页；Env / Args / MCP JSON / 环境变化进入高级折叠区或运行记录详情。Runtime Host 在 AICrew 侧边栏单独成组，点击机器后工作区切到运行时详情。
...
Runtime 不放在顶部成员 Tab，而是放在 AICrew 侧边栏中，作为和“团队成员”并列的基础设施对象：
...
点击成员时，顶部显示 `动态 / 配置`；点击 Runtime 机器时，顶部成员 Tab 不作为主入口，工作区显示该机器的运行时详情。
...
- 预置 `代码工程师`
- 成员详情顶部两 Tab：动态 / 配置
- AICrew 侧边栏 Runtime 分组：本机 / 远程电脑入口
