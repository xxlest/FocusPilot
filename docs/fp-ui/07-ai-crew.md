...
侧边栏使用二级分段切换 `成员 / Runtime`。默认进入成员列表；切到 Runtime 后才展示本机、远程电脑和云端执行环境，避免把“人”和“机器”平铺在同一列表里。
...
工作区采用 Multica 风格的轻量结构。顶部只服务当前成员；Runtime 作为基础设施对象放到侧边栏的二级分段里。

1. `动态`
2. `配置`

`Tasks`、`指令`、`Skills`、`环境变化` 不再作为一级 Tab 常驻展示：Task 和记录留在动态页；Instructions、Skill、Runtime 绑定进入配置页；Env / Args / MCP JSON / 环境变化进入高级折叠区或运行记录详情。Runtime Host 在 AICrew 侧边栏的 `Runtime` 分段中展示，点击机器后工作区切到运行时详情。
...
Runtime 不放在顶部成员 Tab，也不和成员平铺在同一列表里，而是放在 AICrew 侧边栏的 `成员 / Runtime` 分段切换中：
...
点击 `成员` 分段时，工作区默认回到成员详情并显示顶部 `动态 / 配置`；点击 `Runtime` 分段时，侧边栏只显示机器列表，点击机器后工作区显示该机器的运行时详情。

### 3.4 运行时详情

运行配置页右侧只展示当前机器详情；机器切换放在 AICrew 侧边栏 Runtime 分段里，避免工作区再嵌一层侧栏。

```
┌─ MacBook-Pro-10.local ───────────────────────────────────┐
│ 本地 · 这台机器 · 在线 · daemon 019e6da2                 │
│ [View logs] [Restart] [Stop]                            │
│                                                          │
│ Runtime        健康度  智能体      工作负载  费用  CLI     │
│ Claude         在线    代码工程师  空闲      $0.20 0.3.11 │
│ Codex          在线    -           空闲      $0.13 0.3.11 │
│ Cursor         在线    -           空闲      -     0.3.11 │
│ Gemini         在线    -           空闲      -     0.3.11 │
│                                                          │
│ Claude 配置 / 最近运行 / 最近日志                         │
...
| 查看运行时 | 侧边栏点击 Runtime 机器 | 按本机 / 远程电脑 / 云端分组展示机器、执行器、最近运行和最近日志 |
...
- 预置 `代码工程师`
- 成员详情顶部两 Tab：动态 / 配置
- AICrew 侧边栏 `成员 / Runtime` 分段切换：Runtime 分段展示本机 / 远程电脑入口
- Runtime Pool 与 Runtime 绑定选择器
- 运行时页：侧栏本机 / 远程电脑分组，工作区展示执行器列表、最近运行、最近日志、日志入口
