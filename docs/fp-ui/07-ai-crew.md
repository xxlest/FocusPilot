...
Tasks 展示与该成员关联的 Focus 任务，按状态分组。每行有 `data-focus-task-id`，点击可跳转到 Focus 页面。Tasks 和 Focus 一一对应。

```
┌─ 关联任务                    与 Focus 一一对应 · 点击跳转  ─┐
│                                                          │
│ ⏳ 进行中                                                │
│ TES-10  Auth 模块重构        代码工程师  12m03s  [→ Focus] │
│                                                          │
│ 📋 待办                                                  │
│ TES-8   侧边栏 UI 优化       代码工程师  -       [→ Focus] │
│                                                          │
│ 📝 待规划                                                │
│ TES-7   性能监控接入          -          -       [→ Focus] │
│                                                          │
│ ✅ 已完成                                                │
│ TES-12  任务执行间隔优化     代码工程师  7m18s   [→ Focus] │
│ TES-11  调研核心功能以及竞品 代码工程师  5m51s   [→ Focus] │
│ TES-9   二级测试 issue       代码工程师  1m01s   [→ Focus] │
└──────────────────────────────────────────────────────────┘
[查看全部记录]
```
...
| 分组 | 含义 | 视觉 |
|------|------|------|
| ⏳ 进行中 | 正在执行的任务 | amber 标题色 |
| 📋 待办 | 已分配待执行 | muted 标题色 |
| 📝 待规划 | 尚未分配智能体 | dim 标题色 |
| ✅ 已完成 | 执行完毕 | green 标题色 |

每行列：编号（mono 字体）/ 标题 / 智能体 / 耗时 / `→ Focus` 跳转按钮。
...
| 最近日志 | daemon / executor 摘要日志。`View logs` 按钮打开完整记录 |

#### 3.2.2 配置 Tab

配置页采用子 Tab 切换。4 个子 Tab：
