...
Task 行必须保留 Focus 定位能力：点击 Task 标题或 Focus 编号，切换到 Focus 页面并选中对应项目节点；若 Task 已被删除，显示“原 Task 已不存在”，但仍可打开运行记录。

### 3.3 指令

指令页展示当前成员执行时会合并进入上下文的指令来源：
...
每次运行开始时，实际生效的指令会写入 `CrewRunConfigSnapshot`。历史记录查看的是当时快照，不受后续编辑影响。

### 3.4 Skills

Skills 页展示成员可用 Skill 和最近使用情况：

- 默认 Skill
- 已启用 Skill 列表
- 每个 Skill 的最近运行次数、成功次数、成功率
- 最近使用该 Skill 的 Focus Task

点击 Skill 行时停留在本页并展开详情；点击 Skill 最近成功次数时打开记录列表并过滤 `skill_id + status=success`；点击 Skill 关联 Task 时定位到 Focus 项目。

### 3.5 环境变化

环境变化页展示运行相关配置变更，只能检测本机可见配置：

- Env key 新增 / 修改 / 删除记录；secret value 永远不直接展示
- Args 变化
- MCP JSON 变化摘要
- Runtime 绑定变化
- CLI 版本或 daemon 版本变化
- 本机可检测的工作目录、可执行文件路径变化

远程电脑只展示远程 daemon 主动上报的配置摘要，不从本机扫描远程磁盘。
...
| 查看 Skill 记录 | 点击 Skill 或 Skill 成功次数 | 切到 Skills Tab，或打开记录列表并过滤对应 Skill |
| 查看运行配置 | 顶部切到 `运行配置` | 按本机 / 远程电脑 / 云端分组展示机器和执行器 |
