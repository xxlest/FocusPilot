...
**Tasks Tab 分组规则**：任务按状态分为四组，从上到下依次为 `⏳ 进行中`（amber 状态点）→ `📋 待办`（blue 状态点）→ `📝 待规划`（dim 状态点）→ `✅ 已完成`（green 状态点）。分组标题使用 `.crew-task-group-head`（11px 加粗 + 状态点 + 计数），组间用 `line-soft` 1px 上边框分隔。每组内使用 `.crew-task-row` div 行，列为：编号（等宽字体、dim 色）/ 标题。无独立跳转按钮，点击整行调用 `openCrewTaskInFocus(taskId)` 跳转 Focus 看板 → 定位卡片高亮 → 打开 Task 详情面板。
...
**远程节点未连接空状态**：当远程节点尚未建立连接时，三个 Tab（执行器 / 配置 / 日志）各自显示独立的空状态面板。实现方式为双 DOM 面板（`*-local` + `*-empty`），通过 `display` 切换，不使用 `innerHTML` 替换（避免破坏子 Tab 事件绑定）。执行器 Tab 空状态布局：图标（48px、50% 透明度）→ 节点名（h2）→ 状态 pill（`未连接`）→ 说明文案（13px、dim 色、最大宽度 360px、行高 1.6）→ 两个操作按钮（`配置连接` primary + `安装指南` 普通）。配置/日志 Tab 空状态为简洁卡片：图标 + 一行说明文案。
