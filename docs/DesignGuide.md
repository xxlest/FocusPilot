...
运行时页可以保持信息密度，但必须用表格和摘要卡组织，而不是把所有配置铺成表单。机器列表放在 AICrew 侧边栏 Runtime 分段中；启动命令、工作目录、模型策略、Env key count、MCP 摘要等配置只展示摘要，深层编辑通过”配置”按钮进入。
...
| Runtime | `Runtime` | 本机/远程电脑节点列表、添加节点按钮、检测范围摘要卡 |
...
点击侧边栏 Runtime Tab 中的某个节点后，工作区顶部分段控件动态替换为三个 Tab：

| Tab | 标签文案 | 工作区面板 |
|-----|---------|-----------|
| 执行器 | `执行器` | 节点 hero 头部 + 执行器表格 + 三列摘要卡（配置/最近运行/最近日志） |
| 配置 | `配置` | 子 Tab 切换的节点配置面板 |
| 日志 | `日志` | Daemon 日志列表 |

**面包屑**在 Runtime 视图下更新为 `AICrew / Runtime / {节点名}`。

**执行器 Tab**：顶部为节点 hero（节点名 + 副行摘要 + 在线状态 pill + View logs / Restart / Stop 按钮）；下方为 `runtime-table` 表格，列为：执行器（图标 + 名称）/ 健康度（pill）/ 智能体 / 工作负载 / 费用·7天 / CLI 版本（等宽字体）/ 配置按钮。表格下方使用 `grid cols-3` 展示选中执行器配置、最近运行、最近日志三张摘要卡。

**Runtime 配置 Tab 子 Tab 结构**：与智能体配置类似，使用独立 `seg` 分段控件，左对齐。

| 子 Tab | 标签文案 | 内容 |
|--------|---------|------|
| 基础信息 | `基础信息` | 机器名、类型、Daemon ID、App 版本，readonly 字段 |
| 执行器 | `执行器` | 检测模式、检测频率、已发现 CLI |
| 环境变量 | `环境变量` | 节点级 secret-row 列表，全局 pill 标记 |
| 自定义参数 | `自定义参数` | 节点级 Args 输入框 + 合并说明（11px、dim 色） |

**远程节点未连接空状态**：当远程节点尚未建立连接时，三个 Tab（执行器 / 配置 / 日志）各自显示独立的空状态面板。实现方式为双 DOM 面板（`*-local` + `*-empty`），通过 `display` 切换，不使用 `innerHTML` 替换（避免破坏子 Tab 事件绑定），统一由 `setRuntimeHostAvailability(isLocal)` 函数管理三组面板。执行器 Tab 空状态布局：图标（48px、50% 透明度）→ 节点名（h2）→ 状态 pill（`未连接`）→ 说明文案（13px、dim 色、最大宽度 360px、行高 1.6）→ 两个操作按钮（`配置连接` primary + `安装指南` 普通）。配置/日志 Tab 空状态为简洁卡片：图标 + 一行说明文案。
