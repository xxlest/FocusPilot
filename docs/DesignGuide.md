...
团队管理型页面默认不得暴露超过三层一级导航。AICrew V1 顶部只保留成员级 `动态 / 配置` 两个 Tab：动态优先展示最近工作和统计；配置页只展示高频字段；Env、Args、MCP JSON、环境变化等低频内容必须折叠在高级配置或记录详情里。Runtime 是基础设施对象，放在 AICrew 侧边栏的 `成员 / Runtime` 二级分段中，不作为成员顶部 Tab，也不和成员平铺混在同一列表里。除预置代码工程师外，其他角色默认作为模板出现，避免侧边栏看起来像已经配置完成的真实成员。

运行时页可以保持信息密度，但必须用表格和摘要卡组织，而不是把所有配置铺成表单。机器列表放在 AICrew 侧边栏 Runtime 分段中；工作区固定展示执行器表格、选中执行器配置、最近运行、最近日志四块。启动命令、工作目录、模型策略、Env key count、MCP 摘要等配置只展示摘要，深层编辑通过“配置”按钮进入。
