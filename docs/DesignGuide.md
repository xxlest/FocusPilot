...
**Runtime Header**（CSS class `crew-detail-header`）：节点名 + 副行摘要（`5 个运行时 · 5 个在线 · 全部空闲 · daemon v0.3.11 · 最后心跳 12s 前`）+ 在线状态 pill + 操作按钮（View logs / Restart / Stop）。Header 展示 daemon 版本，不展示单个执行器 CLI 版本（各执行器版本可能不同，CLI 版本可通过 tooltip 或日志详情查看）。
