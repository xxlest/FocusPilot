...
| Tasks | `Tasks` | 按状态分组的任务表格，点击行跳转 Focus |
| 配置 | `配置` | 子 Tab 切换的配置面板 |

**Tasks Tab 分组规则**：任务按状态分为四组，从上到下依次为 `⏳ 进行中`（amber 色标题）→ `📋 待办`（muted 色标题）→ `📝 待规划`（dim 色标题）→ `✅ 已完成`（green 色标题）。分组标题使用 11px 大写字母 + 0.5px 字间距，padding `10px 14px 4px`。组间用 `line-soft` 1px 上边框分隔。每组内使用 `runtime-table` 表格，列为：编号（等宽字体、dim 色）/ 标题（加粗）/ 智能体 / 耗时 / 操作按钮（`→ Focus`，11px）。整行 `cursor: pointer`，点击跳转到 Focus 对应 Task。
...
**远程节点未连接空状态**：当远程节点尚未建立连接时，工作区显示居中空状态卡片。布局为：图标（48px、50% 透明度）→ 节点名（h2）→ 状态 pill（`未连接`）→ 说明文案（13px、dim 色、最大宽度 360px、行高 1.6）→ 两个操作按钮（`配置连接` primary + `安装指南` 普通）。整体使用 `text-align: center`，内边距 `60px 24px`。
