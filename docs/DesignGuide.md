...
团队管理型页面默认不得暴露超过三层一级导航。AICrew V1 采用 `动态 / 配置 / 运行时` 三 Tab：动态优先展示最近工作和统计；配置页只展示高频字段；Env、Args、MCP JSON、环境变化等低频内容必须折叠在高级配置或记录详情里。除预置代码工程师外，其他角色默认作为模板出现，避免侧边栏看起来像已经配置完成的真实成员。
