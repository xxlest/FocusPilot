...
本执行模型只作用于 `item_role = executable | hybrid` 的**叶子任务**。容器节点（`item_role = container`，对应 PRD §3.1「容器执行 = 递归执行子节点」）在 V1 **不参与自动调度**：

- 容器节点不可拖入「待办」触发执行（看板本就只展示 executable/hybrid，见 04-studio §7.1）。
- PRD §3.1 描述的「拖容器 → 递归把子任务依次入队执行」为 **V2 能力**，本期不实现。
- V1 下用户对容器的"执行"= 手动把其下的叶子任务逐个拖入「待办」。
...
5. **回复时序随状态分流**：
   - 任务在「审核中」（Agent 已停、等人）时回复 → 立即触发新一轮执行，任务弹回「进行中」。
...
- **「审核中」是强制的人工确认关卡（必经站，非终点）**：无论是否配置自动评估，每轮自动执行跑完都落「审核中」等人；它会与「进行中」来回，真正的终点是「已完成」。删除原"是否需要验收"开关。
