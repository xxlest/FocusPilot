...
> **⚠️ 已被取代（2026-06-22 后续迭代）**：本文所述「失败条件重试 / `msg_state` / 气泡内嵌 ↻」设计，已整体演进为 **「对话操作栏（复制·重新生成）」** 模型——每条 agent 回复带 **📋 复制**、最后一条带 **🔄 重新生成**（任务态 = `retryTask`，聊天态 = 重发上一轮），**不再依赖失败条件、不再引入 `msg_state`**，并移除对话流内「查看 Diff / 日志」。权威现状见 [DesignGuide §5.10](../../DesignGuide.md)、[04-studio.md](../../fp-ui/04-studio.md) 与原型 commit `a9abb44`。下文保留为设计过程快照。
